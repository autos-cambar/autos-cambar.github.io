...
function openFiltersModal() {
  ...
  buildFilterOptions(); // ahora solo construye "Marca" (dinámico)
...
/**
 * Ahora:
 * - Tipo, Orden, Caja, Documentos, Negocio: están en HTML (estáticos)
 * - Marca: se genera desde INVENTORY (dinámico)
 */
let cachedMakesHTML = null;
...
function buildFilterOptions() {
  ...
  if (!cachedMakesHTML) {
    ...
    cachedMakesHTML = makes.map(m => chipHTML({ group: "make", value: m })).join("")
      || `<div class="muted" style="font-size:12px;">Sin opciones</div>`;
  }

  if (el.filterMakes) el.filterMakes.innerHTML = cachedMakesHTML;

  // Los chips ya existen en HTML, pero los listeners no.
