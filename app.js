...
/**
 * Valores con sentido (UX claro)
 * - Marca: dinámica (desde INVENTORY)
 * - Caja: manual / automática / shiftronic
 * - Documentos: se filtra contra docs y/o features (por resiliencia)
 * - Negocio: vende/recibe/para inscripción (cubre "Para inscribir" también)
 */
const FILTER_VALUES = {
  gearbox: [
    { value: "manual", label: "Manual" },
    { value: "automatica", label: "Automática" },
    { value: "shiftronic", label: "Shiftronic" }
  ],
  docs: [
    { value: "Dekra 2026", label: "Dekra 2026" },
    { value: "Marchamo 2026", label: "Marchamo 2026" },
    { value: "RTV 2026", label: "RTV 2026" },
    // soporta inventarios donde alguien ponga "al día"
    // { value: "Dekra al día", label: "Dekra al día" },
    // { value: "Marchamo al día", label: "Marchamo al día" }
  ],
  deal: [
    { value: "VENDO", label: "VENDO" },
    { value: "RECIBO", label: "RECIBO" },
    { value: "para_inscripcion", label: "Para inscripción" }
  ]
};
...
function buildFilterOptions() {
  ...
  if (cachedFilterHTML) {
    ...
    el.filterGearbox.innerHTML = cachedFilterHTML.gearbox;
    el.filterDocs.innerHTML = cachedFilterHTML.docs;
    el.filterDeal.innerHTML = cachedFilterHTML.deal;
  ...
  const makes = uniq(INVENTORY.map(v => v.make).filter(Boolean)).sort((a, b) => compareText(a, b));
  const makesHTML = makes.map(m => chipHTML({ group: "make", value: m })).join("") || `<div class="muted" style="font-size:12px;">Sin opciones</div>`;

  const gearboxHTML = FILTER_VALUES.gearbox
    .map(g => chipHTML({ group: "gearbox", value: g.value, label: g.label }))
    .join("");

  const docsHTML = FILTER_VALUES.docs
    .map(d => chipHTML({ group: "doc", value: d.value, label: d.label }))
    .join("");

  const dealHTML = FILTER_VALUES.deal
    .map(d => chipHTML({ group: "deal", value: d.value, label: d.label }))
    .join("");

  el.filterTypes.innerHTML = typesHTML;
  el.filterSort.innerHTML = sortHTML;
  el.filterMakes.innerHTML = makesHTML;
  el.filterGearbox.innerHTML = gearboxHTML;
  el.filterDocs.innerHTML = docsHTML;
  el.filterDeal.innerHTML = dealHTML;
...
function matchesDeal(item, dealKey) {
  ...
  // Canonical: "Para inscripción" debe cubrir ambos labels (porque el mundo es imperfecto)
  if (dealKey === "para_inscripcion") {
    return itemHasFeatureOrDoc(item, "Para inscripción") || itemHasFeatureOrDoc(item, "Para inscribir");
  }
